...
class ToolsDB2(QtWidgets.QWidget):
    ...
    def storage_to_form(self, dict_storage):
        ...
        # Migration: convert legacy string values for tools_cutout_gap_type to integers
        # Search in top-level and nested 'data'
        keys_to_check = [dict_storage]
        if 'data' in dict_storage:
            keys_to_check.append(dict_storage['data'])

        for k_dict in keys_to_check:
            if 'tools_cutout_gap_type' in k_dict:
                val = k_dict['tools_cutout_gap_type']
                if val == 'b':
                    k_dict['tools_cutout_gap_type'] = 0
                elif val == 'bt':
                    k_dict['tools_cutout_gap_type'] = 1
                elif val == 'mb':
                    k_dict['tools_cutout_gap_type'] = 2
