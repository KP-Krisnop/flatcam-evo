...
class ToolMilling(Excellon, AppTool):
    ...
    def on_tool_add(self, dia=None, new_geo=None):
        ...
        for db_tool, db_tool_val in tools_db_dict.items():
            ...
            truncated_db_tooldia = self.app.dec_format(float(db_tooldia), self.decimals)
            low_limit = float(db_tool_val['data']['tol_min'])
            high_limit = float(db_tool_val['data']['tol_max'])

            # we need only tool marked for Milling Tool (Geometry Object)
            if db_tool_val['data']['tool_target'] not in [0, 1]:     # [General, Milling]
                continue

            # if we find a tool with the same diameter in the Tools DB just update its data
            if truncated_tooldia == truncated_db_tooldia:
                tool_found += 1
    ...
    def generate_cnc_job_handler(self, geo_obj=None, outname=None, tools_dict=None, tools_in_use=None,
        ...
        for tool_uid in tools_dict:
            for opt, opt_val in self.app.options.items():
                if opt.startswith('tools_mill_'):
                    if opt not in tools_dict[tool_uid]['data']:
                        if opt in geo_obj.obj_options:
                            tools_dict[tool_uid]['data'][opt] = deepcopy(geo_obj.obj_options[opt])
                        else:
                            tools_dict[tool_uid]['data'][opt] = deepcopy(opt_val)
        ...
        def job_init_single_geometry(new_cncjob_obj, app_obj):
            ...
            for tool_uid_key in used_tools:
                ...
                # Tooldia update
                if 'tools_mill_tooldia' in tools_dict[tool_uid_key]['data']:
                    tooldia_val = app_obj.dec_format(
                        float(tools_dict[tool_uid_key]['data']['tools_mill_tooldia']), self.decimals)
                else:
                    tooldia_val = app_obj.dec_format(
                        float(tools_dict[tool_uid_key]['tooldia']), self.decimals)
                ...
                spindle_dir = tools_dict[tool_uid_key]['data'].get(
                    "tools_mill_spindledir", self.app.options['tools_mill_spindledir'])
                f_plunge = tools_dict[tool_uid_key]['data'].get(
                    "tools_mill_f_plunge", self.app.options['tools_mill_f_plunge'])
                ...
                res, start_gcode = new_cncjob_obj.generate_from_geometry_2(
                    ...
                    tool_no=tool_cnt, is_first=is_first, f_plunge=f_plunge)
...
class MillingUI:
    ...
    def __init__(self, layout, app, name):
        ...
        self.tool_shape_combo.set_value(self.app.options['tools_mill_tool_shape'])
