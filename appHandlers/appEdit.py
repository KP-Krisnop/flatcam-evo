...
class appEditor(QtCore.QObject):
    ...
    def convert_any2gerber(self):
        ...
        def initialize_from_geometry(obj_init, app_obj):
            ...
            # Default buffer value for LineString geometry when no tool diameter is available
            default_buf = 0.01  # small default width

            # Collect geometry: handle multi-geo objects by pulling from tools
            geo_sources = []  # list of (geometry_list, tooldia_or_None)

            has_multigeo = getattr(obj, 'multigeo', False)

            if has_multigeo and hasattr(obj, 'tools') and obj.tools:
                # Multi-geo: each tool has its own solid_geometry
                for tool_uid in obj.tools:
                    tool_data = obj.tools[tool_uid]
                    tool_geo = tool_data.get('solid_geometry', [])
                    tooldia = tool_data.get('tooldia', None)
                    if tool_geo:
                        if not isinstance(tool_geo, (list, tuple)):
                            tool_geo = [tool_geo]
                        geo_sources.append((tool_geo, tooldia))
            else:
                # Single-geo: use solid_geometry directly
                solid_geo = obj.solid_geometry
                if solid_geo:
                    if not isinstance(solid_geo, (list, tuple)):
                        try:
                            solid_geo = list(solid_geo.geoms)
                        except AttributeError:
                            solid_geo = [solid_geo]

                    # Try to get a tool diameter from tools dict if available
                    tooldia = None
                    if hasattr(obj, 'tools') and obj.tools:
                        first_tool = list(obj.tools.values())[0]
                        tooldia = first_tool.get('tooldia', None)

                    geo_sources.append((solid_geo, tooldia))

            if not geo_sources:
                app_obj.log("convert_any2gerber() failed - no geometry found")
                return 'fail'

            aperture_id = 10
            all_solid_geometry = []

            for geo_list, tooldia in geo_sources:
                buf_val = float(tooldia) / 2.0 if tooldia and float(tooldia) > 0 else default_buf

                current_aperture = {
                    'size': float(tooldia) if tooldia else 0.0,
                    'type': 'C' if tooldia and float(tooldia) > 0 else 'REG',
                    'geometry': []
                }

                for geo_orig in geo_list:
                    if isinstance(geo_orig, (Polygon, MultiPolygon)):
                        # Polygon geometry can be used directly
                        if isinstance(geo_orig, MultiPolygon):
                            for poly in geo_orig.geoms:
                                new_elem = {
                                    'solid': poly,
                                    'follow': poly.exterior
                                }
                                current_aperture['geometry'].append(deepcopy(new_elem))
                                all_solid_geometry.append(poly)
                        else:
                            new_elem = {
                                'solid': geo_orig,
                                'follow': geo_orig.exterior
                            }
                            current_aperture['geometry'].append(deepcopy(new_elem))
                            all_solid_geometry.append(geo_orig)
                    elif isinstance(geo_orig, LinearRing):
                        # Convert LinearRing to Polygon
                        poly = Polygon(geo_orig)
                        new_elem = {
                            'solid': poly,
                            'follow': poly.exterior
                        }
                        current_aperture['geometry'].append(deepcopy(new_elem))
                        all_solid_geometry.append(poly)
                    elif isinstance(geo_orig, LineString):
                        if geo_orig.is_ring:
                            # Closed LineString → fill as a Polygon region
                            poly = Polygon(geo_orig)
                            ...
                            # Open LineString → buffer to create a trace
                            buffered = geo_orig.buffer(buf_val, join_style='round', cap_style='round')
                            if buffered and not buffered.is_empty:
                                if isinstance(buffered, MultiPolygon):
                                    ...
                                    current_aperture['geometry'].append(deepcopy(new_elem))
                                    all_solid_geometry.append(buffered)
                    elif isinstance(geo_orig, Point):
                        # Buffer the Point to create a circular pad
                        buffered = geo_orig.buffer(buf_val)
                        if buffered and not buffered.is_empty:
                            new_elem = {
                                'solid': buffered,
                                'follow': geo_orig
                        ...
                        # For any other geometry type, try to handle it
                        try:
                            new_elem = {'solid': geo_orig}
                            if hasattr(geo_orig, 'exterior'):
                                new_elem['follow'] = geo_orig.exterior
                            current_aperture['geometry'].append(deepcopy(new_elem))
                            all_solid_geometry.append(geo_orig)
                        except Exception:
                            pass

                if current_aperture['geometry']:
                    apertures[aperture_id] = current_aperture
                    aperture_id += 1

            # If no apertures were created, fall back to a single REG aperture
            if not apertures:
                apertures[0] = {
                    'size': 0.0,
                    'type': 'REG',
            ...
            obj_init.solid_geometry = MultiPolygon(all_solid_geometry) if all_solid_geometry else []
