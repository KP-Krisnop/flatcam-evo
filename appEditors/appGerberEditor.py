    ...
    def make(self):
        ...
        for geo_shape in self.draw_app.selected:
            ...
            if 'solid' in geometric_data:
                ...
                if hasattr(solid, 'exterior') and solid.exterior is not None:
                    exterior_geo.append(Polygon(solid.exterior))
                elif isinstance(solid, (LineString, LinearRing)):
                    # Buffer the line to create a polygon
                    buffered = solid.buffer(0.0000001)
                    if not buffered.is_empty and hasattr(buffered, 'exterior'):
                        exterior_geo.append(Polygon(buffered.exterior))
